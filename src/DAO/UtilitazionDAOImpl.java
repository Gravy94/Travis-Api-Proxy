...
public class UtilitazionDAOImpl implements DAO {
	...
	public boolean Insert(Utilization utilization) {
		...
		try {
			...
			ps = conn.prepareStatement("INSERT INTO utilizations (teamId, teamName, channelId, channelName, slugRepo, incomingWebhook)VALUES (?,?,?,?,?,?)");
...
//		System.out.println(utilization.toString());
		try {
			ResultSet resultSet;
//			String query = "SELECT * FROM utilizations WHERE teamId = '"+utilization.getTeamId()+"' AND channelId = '"+utilization.getChannelId()+"' ";
//			System.out.println(query);
	 ...
	 * @param utilization
	 * @param MODE = 1 if utilization is completed, MODE = 2 if utilization contains only t_Id and c_Id
	 * @return
	 */
